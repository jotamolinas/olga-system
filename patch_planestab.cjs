...
const targetLoop = `<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10 mx-auto">
          {plans.map(plan => (
            <div 
              key={plan.id}
              className={\`rounded-3xl p-6 flex flex-col relative transition-all duration-300 hover:-translate-y-2 \${plan.recommended ? 'bg-gradient-to-b from-blue-900 to-slate-800 border-2 border-blue-500/50 transform md:-translate-y-6 shadow-xl shadow-blue-900/40' : 'bg-slate-800/80 backdrop-blur-sm border border-slate-700'}\`}
            >
              {plan.recommended && (
              ...
              <h3 className={\`text-xl font-bold mb-2 \${plan.recommended ? 'text-blue-100' : 'text-white'}\`}>{plan.name}</h3>
              
              <div className="flex flex-col gap-2 mb-6 text-sm">
                <div className="flex items-center gap-2 text-slate-300 bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
              ...
              <div className="mb-8 flex-grow">
                {isAnnual ? (
                  <div className="flex flex-col">
                    <span className="text-slate-500 line-through text-sm font-mono">Gs. {plan.annualBasePrice}</span>
                    <div className="text-3xl font-black text-white font-mono">
                      Gs. {plan.annualDiscountedPrice}
                    </div>
                    <span className={\`text-xs \${plan.recommended ? 'text-blue-300/70' : 'text-slate-400'}\`}>/ año</span>
                  </div>
                ) : (
                  <div className="flex flex-col">
                    ...
                    <span className={\`text-xs \${plan.recommended ? 'text-blue-300/70' : 'text-slate-400'}\`}>/ mes</span>
                  </div>
                )}
              </div>
              ...
              <ul className="space-y-3 mb-8 flex-grow">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className={\`flex gap-3 text-sm items-start \${plan.recommended ? 'text-blue-100' : 'text-slate-300'}\`}>
                    <CheckCircle2 className={\`w-4 h-4 shrink-0 mt-0.5 \${plan.recommended ? 'text-blue-400' : 'text-emerald-500'}\`} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              <button 
                ...
                className={\`w-full font-bold py-3.5 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2 \${plan.recommended ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/30' : 'bg-slate-700 hover:bg-slate-600 text-white'}\`}
              >
                Seleccionar Plan
...
const replacementLoop = `<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10 mx-auto">
          {plans.map(plan => {
